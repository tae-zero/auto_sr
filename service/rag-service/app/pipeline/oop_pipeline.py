...
@dataclass
class Paths:
    csv_questions: Optional[Path] = None
...
@dataclass
class Options:
    do_batch_rag: bool = False
    rag_top_k: int = 5
    rag_collections: List[str] = field(default_factory=lambda: ["sr","standards"])
...
def main():
    ...
    p.add_argument("--csv_questions", default="")
    ...
    p.add_argument("--do_batch_rag", action="store_true")
    p.add_argument("--rag_top_k", type=int, default=5)
    p.add_argument("--rag_collections", nargs="*", default=["sr","standards"])
    ...
    paths = Paths(
        csv_questions=Path(a.csv_questions) if a.csv_questions else None,
    ...
    opts = Options(
        do_batch_rag=bool(a.do_batch_rag and paths.csv_questions),
